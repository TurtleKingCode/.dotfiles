...
local timezone_sign, timezone_offset_hours_s, timezone_offset_minutes_s = tostring(os.date('%z')):match('([-+])(%d%d)(%d%d)')
...
function M.get_calendar_list(opts)
	...
	if opts and opts.refresh then
		_cache_calendar_list = nil
	end

	if _cache_calendar_list then
		return _cache_calendar_list
	end
...
function M.calendar_list_show(opts)
	coroutine.wrap(function()
				...
				iter(lines):map(
					...
					function(calendar_info)
						...
						if not is_new then
							...
							assert(cached_calendar, ('The calendar with id `%s` is not in cache. Maybe you modified it by acciddent'):format(id))
							assert(summary, ('The calendar with id `%s` has no summary'):format(id))
							calendars_by_id[id] = nil

							local edit_diff = {}
							if summary ~= cached_calendar.summary then
								edit_diff.summary = summary
							end
						...
						elseif diff.type == 'delete' then
							coroutine.wrap(function()
								...
								for j, calendar in ipairs(_cache_calendar_list.items) do
									if calendar.id == diff.cached_calendar.id then
										table.remove(_cache_calendar_list.items, j)
									end
								end
		...
		local lines = iter(calendar_list.items):map(
			---@param calendar CalendarListEntry
			function(calendar)
				...
				if calendar.accessRole == 'reader' then
					highlighters[calendar.id .. 'deprecated'] = { pattern = '%f[%w]()' .. calendar.summary .. '()%f[%W]', group = 'DiagnosticDeprecated' }
				end
...
function M.edit_calendar(diff)
	...
	if diff.summary then
		cached_calendar.summary = diff.summary
	end
...
function M.get_events(calendar_list, opts)
	...
	if opts.should_query_single_events == nil then
		opts.should_query_single_events = true
	end
	...
	do
		...
		if end_yday < start_yday then
			end_yday = end_yday + 365
		end
...
function CalendarView:w_in_m(year, month)
	...
	if (first_day.wday == 1 and month ~= 2) or (first_day.wday == 7 and last_day.day == 31) then
		return 6
	end
	return 5
end
...
function CalendarView:year(year, height)
	if height < self.header_required_height then
	...
	local a = iter(digits):map(function(digit)
		...
		function(line)
			...
			if line:match('^/[^ ]+') then
				...
				elseif #fields == 3 then
					...
					should_abort = true
					vim.notify(("The event with id %s doens't have enoght fields to be parsed"):format(id), vim.log.levels.ERROR)
				end
			else
		...
		function(event_info)
			...
			if not is_new then
				...
				if not cached_event then
					should_abort = true
					vim.notify(('The event with id `%s` is not in cache. Maybe you modified it by acciddent'):format(id), vim.log.levels.ERROR)
					return
				end
					...
					coroutine.wrap(function()
						...
						for _, events in pairs(_cache_events) do
							for j, event in ipairs(events) do
								if event.id == diff.cached_event.id then
...
function CalendarView:show(year, month, opts)
	...
	if not vim.tbl_isempty(self.day_bufs) then
		self.day_bufs = {}
	end
	if not vim.tbl_isempty(self.cal_bufs) then
		self.cal_bufs = {}
	end
	if not vim.tbl_isempty(self.cal_wins) then
		self.cal_wins = {}
	end
	if self.year_buf then
		self.year_buf = nil
	end
	if self.year_win then
		self.year_win = nil
	end
	if self.d_in_w_border_win then
		self.d_in_w_border_win = nil
	end
	if self.y_m_border_win then
		self.y_m_border_win = nil
	end
	if self.month_buf then
		self.month_buf = nil
	end
	if self.d_in_w_border_buf then
		self.d_in_w_border_buf = nil
	end
	if self.y_m_border_buf then
		self.y_m_border_buf = nil
	end
	if self.month_win then
		self.month_win = nil
	end
	...
	if x_first_day_month <= 0 then
		x_first_day_month = x_first_day_month + 7
	end
	do
		local i = 1
		for y = 1, w_in_m do
	...
	coroutine.wrap(function()
		...
		if self.d_in_w_border_win then
			table.insert(all_wins, self.d_in_w_border_win)
		end
		if self.y_m_border_win then
			table.insert(all_wins, self.y_m_border_win)
		end

		api.nvim_create_autocmd('WinClosed', {
			pattern = iter(all_wins):map(function(win)
				return tostring(win)
			end):totable(),
			callback = function()
			...
			for x = 1, self.d_in_w do
				...
				api.nvim_create_autocmd('BufReadCmd', {
					...
					callback = function()
						...
						local events_text = iter(day_events):map(function(event)
							...
							if not event.start.dateTime then
								return ('/%s %s%s'):format(event.id, event.summary, recurrence)
							end
				...
				iter(day_events):each(
					...
					function(event)
						...
						if not calendar or not event.summary then
							return
						end
...
function M.create_event(calendar_id, diff)
	...
	local data = vim.json.encode({ start = diff.start, ['end'] = diff['end'], summary = diff.summary, recurrence = diff.recurrence })
	local tmp_name = os.tmpname()
	local tmp_file = io.open(tmp_name, 'w')
	assert(tmp_file)
	tmp_file:write(data)
...
function M.edit_event(calendar_id, diff)
	...
	if diff.summary then
		cached_event.summary = diff.summary
	end
	if diff.recurrence then
		cached_event.recurrence = diff.recurrence
	end
	if diff.start then
		cached_event.start = diff.start
	end
	if diff['end'] then
		cached_event['end'] = diff['end']
	end
	if diff.reminders then
		cached_event.reminders = diff.reminders
	end
	if diff.description then
		cached_event.description = diff.description
	end
	...
	vim.system(
		{
			...
			'--header',
			('Authorization: Bearer %s'):format(token_info.access_token),
			('https://www.googleapis.com/calendar/v3/calendars/%s/events/%s'):format(uri_encode(calendar_id), uri_encode(diff.cached_event.id)),
		},
		{ text = true },
		vim.schedule_wrap(function(result)
			assert(result.stderr == '', result.stderr)
...
function M.get_event(calendar_id, id, opts)
	...
	if _cache_event[id] then
		return _cache_event[id]
	end
...
local function event_write(buf, win, calendar_list, day_events, opts)
	...
	if recurrence and #recurrence == 1 and recurrence[1] == '' then
		recurrence = nil
	end
	...
	if vim.tbl_isempty(reminders.overrides) then
		reminders.overrides = nil
	end
	local description ---@type string|nil
	if lines[5] then
		description = table.concat(vim.list_slice(lines, 6), '\n')
	end
...
function M.event_show(calendar_list, day_events, opts)
	coroutine.wrap(function()
		...
		local reminders = ('%s%s%s'):format(
			...
			sep,
			iter(recurring_event.reminders.overrides or {}):map(function(o)
